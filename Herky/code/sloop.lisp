...
;;rascal.ics.utexas.edu:/usr2/ftp/pub/sloop.lisp
...
(in-package "SLOOP"  :use '(USER LISP))  
...
(defvar *type-check* t "If t adds a type check on bounds of from loop if there
is and automatic declare")
...
;;to do
;;Fix (declare (joe (type (array fixnum)))) type for declarations.
...
;;some cl implementations lack define-setf-method and others have already defined this.
;;so we will change the definition of desetq to not use setf.
...
;  (cond ((symbolp form) `(setq ,form ,val))
;	(t
	    ...
	    (cond ((symbolp form)
		  ...
		  ((consp form)
		   `(progn ,(desetq1 (car form) `(car ,val))
			   ,@ (if (consp (cdr form)) (list(desetq1 (cdr form) `(cdr ,val)))
...
;; (defmacro loop-finish () `(go finish-loop))
		...
		(list value)))
	       (t  (let (chk)
			 ...
			 (or
			  (typep
			   ,(if *type-test-limit*
				(subst (or chk value) 'the-value *type-test-limit*)
			      (or chk value))
			   ',type)
			  (type-error))
			 ,(or chk value)))))))
...
(defun parse-loop-for ( &aux  inc  from-data)
  ...
  (let* ((var (loop-pop)) test incr)
	...
	((upfrom from)
           ;from-data '(var begin end  incr direction or-eql)
          (setq from-data (add-from-data from-data var (loop-pop) nil  nil '+)))
	(downfrom
	  (setq from-data  (add-from-data from-data var (loop-pop) nil  nil '-)))
	(by
         (setq inc (loop-pop))
	 ...
	 (cond (from-data
		(setq from-data (add-from-data from-data nil nil nil inc)))
	       (t (assert (eq (car (third incr)) 'cdr))
		  (setq incr
			`(setf ,(second incr) (, inc ,(second incr)))))))
	(below
	  (setq from-data (add-from-data from-data var nil (loop-pop) nil '+)))
	(above
	  (setq from-data (add-from-data from-data var nil (loop-pop) nil '-)))
	(to
	 (setq from-data (add-from-data from-data var nil (loop-pop) nil nil t)))
	(:sloop-for (parse-loop-macro (translate-name v) :sloop-for var )
		    (return 'done))
        (:sloop-map (parse-loop-map (translate-name v) var ) (return nil))
	(t(or ; (null *loop-form*)
	      (loop-un-pop))
	  (return 'done))))
...
;it will not get the correct initialization.  But since we have such generality in the
;our definition of a for construct, it is unlikely that all people who define
;This is why we use a different name
