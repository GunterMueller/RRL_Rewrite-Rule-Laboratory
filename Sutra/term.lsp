...
(defun is-ground (term)
  ...
  (cond ((null term) t)
        ((variablep term) nil)
	(t (loop for xa in (args-of term) always (is-ground xa)))))
   ...
   (cond ((variablep term) (ncons term))
	 ((memq (op-of term) '(all exist))
	  (delete (first-arg term) (free-vars (second-arg term))))
         (t (rem-dups
	     (loop for this in (args-of term) nconc (free-vars this))))))
...
(defun subtat (pos term)
  ...
  (if pos then (loop for i in pos do (setq term (nthsubt i term))))
  term)

(defun rplnthsubt-in-by (n term t1)
  ;  Replaces then Nth top-level subterm of TERM by T1.
  (make-term (op-of term)
	     (loop for xa in (args-of term) as i from 1 collect
...
(defun repvarsin (term) 
  (if (and v_binds (nequal v_binds '(nil)))
      (applysubst v_binds term) term))
...
(defun new-vars (vars1 vars2)
  ...
  (let ((vars (append vars1 vars2)))
     ...
     (loop for v1 in vars1 if (member v1 vars2) 
...
(defun new-var (vars)
  ...
  (prog (v1)
     (loop while (setq v1 (newsym 'x)) do
        (if (> (getchar v1 2) 9) then (return (setq v1 nil))
	 elseif (not (member v1 vars)) then (return)))
     (if v1 then (return v1))
     (loop while (setq v1 (newsym 'y)) do
        (if (> (getchar v1 2) 9) then (return (setq v1 nil))
	 elseif (not (member v1 vars)) then (return)))
     (if v1 then (return v1))
     (loop while (setq v1 (newsym 'z)) do
	 (if (not (member v1 vars)) then (return)))
     (return v1)))

(defun is-subt (t1 t2)
  ;  Returns t if T1 is a subterm of T2; otherwise, nil.
  (if (equal t1 t2) then t
    else (loop for arg in (args-of t2) thereis (is-subterm t1 arg))))
...
(defun occurs-in (var term)
  (if (eq var term) t
	...
	(loop for arg in (args-of term) thereis (occurs-in var arg)))))
...
(defun is-sub-nonvar-term (t1 t2)
  ...
  (if (variablep t2) then nil
      ...
      then (if (args-of t2) then
	       (loop for arg1 in (set-diff (args-of t1) (args-of t2))
		     always (loop for arg2 in (set-diff (args-of t2) (args-of t1))
				  thereis (is-sub-nonvar-term arg1 arg2))))
      else (loop for arg2 in (args-of t2) thereis (sub-or-eq-term t1 arg2))))
...
(defun sub-or-eq-term (t1 t2)
  ...
  (if (variablep t1) then t
      ...
      elseif (same-op t1 t2)
      then (loop for arg1 in (args-of t1) 
		 always (loop for arg2 in (args-of t2) 
			      thereis (sub-or-eq-term arg1 arg2)))
      else (loop for arg2 in (args-of t2)
...
(defun rename-var1 (t1 name &optional vars)
   ...
   (cond ((variablep t1) (loop for xa = (newsym name) then (newsym name) 
			         if (not (member xa vars)) return xa))
	 (t (make-term (op-of t1) 
		       (loop for arg in (args-of t1) 
...
(defun is-limited (t1 op-set)
  ...
  (cond ((variablep t1) t)
        ((not (member (op-of t1) op-set)) nil)
        ((null (args-of t1)) t)
        (t (loop for xa in (args-of t1) always (is-limited xa op-set)))))
...
(defun groundp (term &optional varlist)
  (cond
    ((variablep term) (member term varlist))
    (t (loop for x in (args-of term) always (groundp x varlist)))))
...
(defun one-type-all-vars (term type)
  ...
  (loop for xa in (get-codomain-types (op-of term))
